...
import { SPINALAPP_TYPE, SPINALTWIN_ADMIN_SERVICE_USER_RELATION_NAME, USER_BASE_EMPTY, USER_LIST, USER_NOT_FOUND } from '../constant';
...
export class SpinalTwinAdminUser {
  ...
  * @memberof SpinalTwinAdminUser
  */
  public async getUser(id: string, email?: string, password?: string): Promise<SpinalTwinUser> {
    let context = await spinalTwinGraph.getContext(USER_LIST);
    if (typeof email === 'string' && typeof password === 'string')
      return this.findUserWithEmailPassword(email, password);
    return SpinalGraphService.getChildren(context.info.id.get(), [SPINALTWIN_ADMIN_SERVICE_USER_RELATION_NAME])
      .then((children: any[]) => {
  ...
  * @memberof SpinalTwinAdminUser
  */
  private async findUserWithEmailPassword(email: string, password: string): Promise<SpinalTwinUser> {
    let context = await spinalTwinGraph.getContext(USER_LIST);
    return SpinalGraphService.getChildren(context.info.id.get(), [SPINALTWIN_ADMIN_SERVICE_USER_RELATION_NAME])
      .then((children: any[]) => {
        if (children.length < 0) {
