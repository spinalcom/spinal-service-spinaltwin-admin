        ...
        it('should create a new user profile', async function (done) {
              try {
                const context = await spinalTwinGraph.getContext("UserProfileList");
                  ...
                  const data = await spinalUserProfileService.createUserProfile(Obj, context.info.id.get());
                  assert.exists(data);
      ...
      describe("GetUserProfile", function () {
          it('should get a User Profile', async function (done) {
                try {
                    ...
                    const data = await spinalUserProfileService.getUserProfile(userProfileId);
                    assert.exists(data);
                    done()
                } catch (e) {
                    done(e)
          ...
          it('should update a user profile', async function (done) {
                try {
                  ...
                  let userProfile = "25478965";
                    const data = await spinalUserProfileService.updateUserProfile(testObj, userProfile);
